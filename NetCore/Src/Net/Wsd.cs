...
namespace VeriFactu.Net
{
  ...
  public static class Wsd
  {
    ...
    /// <summary>
    /// Crea la instancia WebRequest para enviar la petición al web service de la AEAT.
    /// </summary>
    ...
    /// <summary>
    /// Certificado a utilizar en la comunicaciones con la AEAT. Si su valor no está establecido, se intentará cargar el
    /// certificado con los valores establecidos en la configuración de VeriFactu.
    /// </summary>
    ...
    /// <summary>
    /// Devuelve el certificado configurado siguiendo la siguiente jerarquía de llamadas: En primer lugar prueba a
    /// cargar el certificado desde un archivo, si no prueba por el hash y en último lugar prueba por el número de
    /// serie.
    /// </summary>
    /// <returns>
    /// Devuelve el certificado de la  configuración para las comunicaciones.
    /// </returns>
    public static X509Certificate2 GetCertificate()
    {
      ...
      X509Certificate2 cert = GetCertificateByFile(); // 2. Valor en Settings por fichero.
      if(cert != null)
      {
        return cert;
      }
    ...
    /// <summary>
    /// Devuelve el certificado establecido en la configuración por su número de serie..
    /// </summary>
    /// <returns>
    /// Devuelve el certificado de la  configuración por número de serie para las comunicaciones. Si no existe devuelve
    /// null.
    /// </returns>
    public static X509Certificate2 GetCertificateBySerial()
    {
      foreach(X509Store store in new X509Store[] { new X509Store(), new X509Store(StoreLocation.LocalMachine) })
      {
    ...
    /// <summary>
    /// Devuelve el certificado establecido en la configuración por su hash o huella digital.
    /// </summary>
    /// <returns>
    /// Devuelve el certificado de la  configuración por hash para las comunicaciones. Si no existe devuelve null.
    /// </returns>
    ...
    /// <summary>
    /// Devuelve el certificado establecido en la configuración mediante una ruta a un fichero de certificado.
    /// </summary>
    /// <returns>
    /// Devuelve el certificado de la  configuración para las comunicaciones.
    /// </returns>
