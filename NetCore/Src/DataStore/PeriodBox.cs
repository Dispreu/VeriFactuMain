...
namespace VeriFactu.DataStore
{
  ...
  public class PeriodBox
  {
    ...
    /// <summary>
    /// Devuelve todos los envíos de un vendedor para un periodo determinado.
    /// </summary>
    /// <returns>Lista de documentos envíados.</returns>
    private List<Envelope> GetEnvelopes()
    {
      List<Envelope> envelopes = new List<Envelope>();
      string[] envelopeFiles = Directory.GetFiles(EnvelopeDir);
      foreach(string envelopeFile in envelopeFiles)
      {
    ...
    /// <returns>
    /// Diccionario de mensajes enviados a Verifactu de la AEAT de un emisor y periodo determinados.
    /// </returns>
    internal Dictionary<string, DocumentBox> GetDocuments(List<Envelope> envelopes)
    {
      Dictionary<string, DocumentBox> documents = new Dictionary<string, DocumentBox>();
      foreach(Envelope envelope in envelopes)
      {
        IList<object> registros = GetInvoiceRecords(envelope);
        foreach(object registro in registros)
        {
          string recordID = SetRegistro(registro);
    ...
    internal virtual IList<object> GetInvoiceRecords(Envelope envelope)
    {
      ...
      RegFactuSistemaFacturacion regFactuSistemaFacturacion = envelope.Body.Registro as RegFactuSistemaFacturacion;
      ...
      IList<RegistroFactura> registros = regFactuSistemaFacturacion?.RegistroFactura;
      if(registros == null)
      {
      ...
      foreach(RegistroFactura registro in registros)
      {
    ...
    {
      Registro registro = record as Registro;
      if(registro == null)
      {
        throw new InvalidOperationException($"Error: Encontrado un envío de mensaje SOAP con propiedad Body desconocida {registro}");
      }
      RegistroAlta alta = registro as RegistroAlta;
      RegistroAnulacion baja = registro as RegistroAnulacion;
      if(alta != null)
      {
    ...
    /// </summary>        
    public string SellerID => Seller.SellerID;

    /// <summary>
    /// Nombre del vendedor.
    /// </summary>        
    public string SellerName => Seller.SellerName;
    ...
    /// <summary>
    /// Recupera un diccionario de documentos envíados del periodo cuya clave es el IDFactura.
    /// </summary>
