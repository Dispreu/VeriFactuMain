...
{
  /// <summary>
  /// Representa la cola de envío de documentos de un emisor determinado.
  /// </summary>
  public class SellerQueue : SingletonByKey<SellerQueue>
  {
    ...
    /// <summary>
    /// Lista de procesamiento. En esta lista se incluyen todos los elmentos a procesar.
    /// </summary>
    private Queue<InvoiceAction> _InvoiceActions = new Queue<InvoiceAction>();

    /// <summary>
    /// Almacena el momento de finalización del último envío.
    /// </summary>
    private DateTime _LastProcessMoment;

    /// <summary>
    /// Almacena tiempo de espera comunicado por la AEAT en el últinmo envío.
    /// </summary>
    private int _CurrentWaitSecods = 60;
    ...
    /// <summary>
    /// Si es true indica que el envío ya está permitido por haberse superado el último tiempo de espera comunicado por
    /// la AEAT.
    /// </summary>
    internal bool IsAllowedFrom => AllowedFrom < DateTime.Now;

    /// <summary>
    /// Si es true indica que el envío ya está permitido por haberse alcanzado el máximo de registros por lote
    ...
    /// <summary>
    /// Contabiliza los elementos a envíar eliminándolos de la cola de envío y devolviendolos en una lista.
    /// </summary>
    /// <param name="invoiceRetrySends">
    /// Lista de reenvíos. Cuando en cola existen envíos normales y reenvíos no se puede realizar un único envío, ya que
    /// la cabecera difiere en los dos casos. En los reenvíos el bloque 'Cabecera' tiene en 'RemisionVoluntaria' el
    /// bloque 'Incidencia'='S'. En estos casos se procesan los envíos normales dándoles prioridad, que coincida en la
    /// cola un momento de envío en el que únicamente existan reenvíos; En este momento será cuando se envíen estos
    /// registros.
    /// </param>
    ...
    {
      Utils.Log(
        $"Ejecutando por cola ({SellerID}) tras tiempo espera en segundos:" +
                $" {_CurrentWaitSecods} desde {_LastProcessMoment} hasta {AllowedFrom}");
      Debug.Print(
        $"Ejecutando por cola ({SellerID}) tras tiempo espera en segundos:" +
                $" {_CurrentWaitSecods} desde {_LastProcessMoment} hasta {AllowedFrom}");
      int recordCount = 0;
      List<InvoiceAction> processInvoiceActions = new List<InvoiceAction>();
      ...
      bool isOnlyRetrySends = hasRetrySends && !hasInvoiceActions;
      foreach(InvoiceAction invoiceAction in processInvoiceActions)
      {
      ...
      Blockchain.Blockchain blockchainManager = Blockchain.Blockchain.GetInstance(SellerID) as Blockchain.Blockchain;
    ...
    private RespuestaRegFactuSistemaFacturacion Send(List<InvoiceAction> invoiceActions)
    {
      ...
      InvoiceBatch sender = new InvoiceBatch();
      RespuestaRegFactuSistemaFacturacion respuesta = sender.Send(invoiceActions);
      ...
      Utils.Log(
        $"Establecido momento próxima ejecución {SellerID} (LastProcessMoment: {_LastProcessMoment} + " +
    ...
    /// <summary>
    /// Devuelve la instancia correspondiente a la cola de registros de un emisor de facturas.
    /// </summary>
    /// <param name="sellerID">Id. del emisor de factura.</param>
    /// <returns>
    /// Instancia correspondiente a la cola de registros de un emisor de facturas.
    /// </returns>
    ...
    /// <summary>
    /// Añade un elemento a la cola de registros pendientes de envío.
    /// </summary>
    ...
    internal void Add(InvoiceAction invoiceAction)
    {
      if(invoiceAction.Posted)
      {
        throw new InvalidOperationException(
          $"La operación {invoiceAction}" +
                          $" ya está contabilizada y por lo tanto no se puede agregar a la cola.");
      }
      List<string> busErrors = invoiceAction.GetBusErrors();
    ...
    internal void Process()
    {
      ...
      List<InvoiceAction> postedInvoiceActions = Post(out invoiceRetrySends);
      if(invoiceRetrySends.Count > 0)
      {
        foreach(InvoiceAction invoiceRetrySend in invoiceRetrySends)
        {
      ...
      RespuestaRegFactuSistemaFacturacion aeatResponse = Send(postedInvoiceActions);
