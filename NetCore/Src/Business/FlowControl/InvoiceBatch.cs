...
namespace VeriFactu.Business.FlowControl
{
  ...
  public class InvoiceBatch
  {
    ...
    /// </summary>
    private List<InvoiceAction> _InvoiceActions;
    ...
    internal RespuestaRegFactuSistemaFacturacion Send(List<InvoiceAction> invoiceActions)
    {
      ...
      for(int i = 0; i < invoiceActions.Count; i++)
      {
        ...
        {
          ((envelope.Body.Registro as RegFactuSistemaFacturacion).RegistroFactura).Add(
            new RegistroFactura
            {
              Registro = invoiceActions[i].Registro
            });
        }
        last = invoiceActions[i];
      }
      byte[] xml = new XmlParser().GetBytes(envelope, Namespaces.Items);
      string response = last.Send(xml);
      Envelope envelopeRespuesta = last.GetResponseEnvelope(response);
      ...
      RespuestaRegFactuSistemaFacturacion respuesta = (envelopeRespuesta.Body.Registro as RespuestaRegFactuSistemaFacturacion);
      if(respuesta == null)
      {
        Fault fault = (envelopeRespuesta.Body.Registro as Fault);
        if(fault == null)
        {
    ...
    private List<InvoiceAction> Post(List<InvoiceAction> invoiceActions)
    {
      ...
      foreach(InvoiceAction invoice in invoiceActions)
      {
      ...
      Blockchain.Blockchain blockchainManager = Blockchain.Blockchain.Get(SellerID);
    ...
    {
      Dictionary<string, InvoiceAction> invoices = new Dictionary<string, InvoiceAction>();
      foreach(InvoiceAction invoiceAction in invoiceActions)
      {
      ...
      foreach(RespuestaLinea line in aeatResponse.RespuestaLinea)
      {
        string externKey = line.RefExterna;
        InvoiceAction invoice = invoices[externKey];
        Envelope invoiceEnvelope = new Envelope
        {
          Body = new Body
          {
        ...
        RespuestaRegFactuSistemaFacturacion respuesta = invoiceEnvelope.Body.Registro as RespuestaRegFactuSistemaFacturacion;
    ...
    /// <summary>
    /// Contabiliza en la cadena de bloques, envía a la AEAT y devuelve los resultados del lote.
    /// </summary>
    /// <returns>Resultado del guardado del lote.</returns>
    public Dictionary<string, InvoiceAction> Save()
    {
      List<InvoiceAction> postedInvoiceActions = Post(_InvoiceActions);
