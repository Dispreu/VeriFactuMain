...
namespace VeriFactu.Business.Operations
{
  ...
  public class InvoiceQuery
  {
    ...
    /// </summary>
    private static string _Action = "?op=ConsultaFactuSistemaFacturacion";
    ...
    /// <summary>
    /// Obtiene un objeto invoice a partir de un registro de consulta de la AEAT.
    /// </summary>
    /// <param name="registro">
    /// Registro de consulta correspondiente a  una factura.
    /// </param>
    /// <param name="sellerName">Nombre vendedor.</param>        
    /// <returns>
    /// Objeto Invoice creado a partir de un registro de factura de una respuesta de consulta a la AEAT.
    /// </returns>
    private static Invoice GetInvoice(RegistroRespuestaConsultaFactuSistemaFacturacion registro, string sellerName = null)
    {
      Invoice invoice = new Invoice(
        ...
        XmlParser.ToDate(registro.IDFactura.FechaExpedicionFactura),
        $"{registro.IDFactura.IDEmisorFactura}");
      DatosRegistroFacturacion registroAlta = registro.DatosRegistroFacturacion;
      ...
      if(registroAlta.Destinatarios.Count == 1)
      {
        Destinatario destinatario = registroAlta.Destinatarios[0];
    ...
    /// <summary>
    /// Devuelve un objeto Envelope con el filtro para la consulta de facturas emitidas.
    /// </summary>
    /// <param name="year">Año a consultar.</param>
    /// <param name="month">Mes a consultar.</param>
    /// <returns>
    /// Objeto Envelope con el filtro para la consulta de facturas emitidas.
    /// </returns>
    ...
    /// <summary>
    /// Devuelve un objeto Envelope con el filtro para la consulta de facturas recibidas.
    /// </summary>
    /// <param name="year">Año a consultar.</param>
    /// <param name="month">Mes a consultar.</param>
    /// <returns>
    /// Objeto Envelope con el filtro para la consulta de facturas recibidas.
    /// </returns>
    ...
    /// <summary>
    /// Obtiene una lista de objetos Invoice a partir de una respuesta de la AEAT a  una consulta de documentos.
    /// </summary>
    /// <param name="queryAeatResponse">Respuesta consulta AEAT.</param>
    /// <returns>
    /// Lista de objetos Invoice a partir de una respuesta de la AEAT a  una consulta de documentos.
    /// </returns>
    public static List<Invoice> GetInvoices(RespuestaConsultaFactuSistemaFacturacion queryAeatResponse)
    {
      List<Invoice> invoices = new List<Invoice>();
      string sellerName = queryAeatResponse.Cabecera?.ObligadoEmision?.NombreRazon;
      foreach(RegistroRespuestaConsultaFactuSistemaFacturacion registro in queryAeatResponse.RegistroRespuestaConsultaFactuSistemaFacturacion)
      {
    ...
    /// <summary>
    /// Devuelve las facturas emitidas por el NIF facilitado en la propiedad PartyID.
    /// </summary>
    /// <param name="year">Año a consultar.</param>
    /// <param name="month">Mes a consultar.</param>
    ...
    {
      Envelope envelope = GetSalesEnvelope(year, month);
      byte[] xml = new XmlParser().GetBytes(envelope, Namespaces.Items);
      string response = InvoiceActionMessage.SendXmlBytes(xml, _Action);
      Envelope envelopeResponse = Envelope.FromXml(response);
      Fault fault = envelopeResponse.Body.Registro as Fault;
      if(fault != null)
      {
    ...
    {
      Envelope envelope = GetPurchasesEnvelope(year, month);
      byte[] xml = new XmlParser().GetBytes(envelope, Namespaces.Items);
      string response = InvoiceActionMessage.SendXmlBytes(xml, _Action);
      Envelope envelopeResponse = Envelope.FromXml(response);
      Fault fault = envelopeResponse.Body.Registro as Fault;
