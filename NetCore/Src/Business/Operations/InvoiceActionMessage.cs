...
{
  /// <summary>
  /// Representa una acción de alta o anulación de registro en todo lo referente a su envío al web service de la AEAT y
  ...
  public class InvoiceActionMessage : InvoiceActionData
  {
    ...
    /// </summary>
    private static string _Action = "?op=RegFactuSistemaFacturacion";
    ...
    /// <summary>
    /// Devuelve la ruta de almacenamiento de los registros contabilizados y envíados para un vendedor en concreto.
    /// </summary>
    /// <param name="sellerID">
    ...
    /// <summary>
    /// Devuelve la ruta de almacenamiento de repuestas de los registros contabilizados y envíados para un vendedor en
    /// concreto.
    /// </summary>
    /// <param name="sellerID">
    /// Emisor al que pertenece el envío de registro a gestionar.
    /// </param>
    /// <returns>
    /// Ruta respuestas de los registros contabilizados y envíados para un vendedor en concreto.
    /// </returns>
    ...
    /// <summary>
    /// Devuelve la ruta de almacenamiento de respuestas de los registros contabilizados y envíados para un año en
    /// concreto.
    /// </summary>
    /// <param name="year">
    /// Año al que pertenece el envío de registro a gestionar.
    ...
    /// <summary>
    /// Devuelve el nombre completo de un archivo xml de respuesta de la AEAT cuyo envío ha resultado erróneo.
    /// </summary>
    /// <returns>
    /// Nombre completo de un archivo xml de respuesta de la AEAT que ha resultado erróneo.
    /// </returns>
    ...
    /// <summary>
    /// Path de la entrada factura en el directorio de archivado de los datos de la cadena si el documento a resultado
    /// erróneo.
    /// </summary>
    /// <returns>
    ...
    {
      string invoiceEntryFilePath = string.IsNullOrEmpty(CSV) ? GetErrorInvoiceEntryFilePath() : InvoiceEntryFilePath;
      string responseFilePath = string.IsNullOrEmpty(CSV) ? GetErrorResponseFilePath() : ResponseFilePath;
    ...
    /// <summary>
    /// Identificador del eslabón de la cadena asociado a la factura.
    /// </summary>
    public virtual string InvoiceEntryID => Registro?.BlockchainLinkID == null ?
        null :
        $"{Registro.BlockchainLinkID}".PadLeft(20, '0');
...
#pragma warning restore CA1819
    ...
    /// <summary>
    /// Indica si el registro ha sido envíado y la respuesta ha sido procesada de la AEAT.
    /// </summary>
    ...
    public static string SendXmlBytes(byte[] xml, string op = null)
    {
      ...
      using(MemoryStream msXml = new MemoryStream(xml))
      {
        xmlDocument.Load(msXml);
      }
      string url = Settings.Current.VeriFactuEndPointPrefix;
      string action = $"{url}{op}";
    ...
    {
      // Generamos el xml
      byte[] xml = new XmlParser().GetBytes(envelope, Namespaces.Items);
    ...
    public static Envelope SendEnvelope(Envelope envelope)
    {
      // Generamos el xml
      byte[] xml = new XmlParser().GetBytes(envelope, Namespaces.Items);
      string response = SendXmlBytes(xml, _Action);
    ...
    /// <summary>
    /// Serializa como sobre soap un string de respuesta de la AEAT.
    /// </summary>
    ...
    /// <summary>
    /// Devuelve los bytes del XML serializado con los  datos actuales.
    /// </summary>
    /// <returns>
    /// Bytes del XML serializado con los  datos actuales.
    /// </returns>
    public byte[] GetXml()
    {
