...
{
  /// <summary>
  /// Representa una acción de reenvío por fallo técnico en las comunicaciones u otra causa. La factura es correcta, ha
  /// sido contabilizada en la cadena de bloques pero no ha sido enviada a la AEAT.
  /// </summary>
  public class InvoiceRetrySend : InvoiceEntry
  {
    ...
    internal override void SetRegistro()
    {
      ...
      string path = OriginalInvoiceFilePath;
      ...
      Envelope envelope = new Envelope(OriginalInvoiceFilePath);
      ...
      string refExt = (registro as RegistroAlta).RefExterna ?? (registro as RegistroAnulacion).RefExterna;
      ...
      string fechaHoraHusoGenRegistro = (registro as RegistroAlta).OrderedFechaHoraHusoGenRegistro ?? (registro as RegistroAnulacion).OrderedFechaHoraHusoGenRegistro;
    ...
    internal override Envelope GetEnvelope()
    {
      Envelope envelope = new Envelope(OriginalInvoiceFilePath); 
            ...
            (envelope.Body.Registro as RegFactuSistemaFacturacion).Cabecera.RemisionVoluntaria = new RemisionVoluntaria
            {
              Incidencia = "S"
            };
      return envelope;
