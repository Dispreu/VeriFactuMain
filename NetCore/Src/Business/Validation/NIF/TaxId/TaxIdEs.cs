...
{
  /// <summary>
  /// Clase para almacenar, verificar y obtener información de números de identificación fiscal españoles de acuerdo con
  ...
  public class TaxIdEs
  {
    ...
    /// <summary>
    /// Diccionario de códigos de provincia definidos para el NIF de personas jurídicas. No significativo ya.
    /// </summary>
    private static readonly Dictionary<string, string> TaxCCAA = new Dictionary<string, string>
    {
      { "01", "Álava" },
      { "02", "Albacete" },
      { "03", "Alicante" },
    ...
    /// </summary>
    private static readonly Dictionary<string, string> LegalEntities = new Dictionary<string, string>
    {
    ...
    /// </summary>
    private static readonly Dictionary<string, string> Individuals = new Dictionary<string, string>
    {
      {
        "K",
    ...
    /// <summary>
    /// Patrón de expresión regular que identifica códigos de identificación fiscal españoles.
    /// </summary>
    public static string PatternTaxIdentification = @"(\b[A-HJ-NP-SU-Z0-9]{1}[0-9]{7}[A-NP-TV-Z0-9]{1}\b)";

    /// <summary>
    /// Patrón de expresión regular que identifica códigos de identificación fiscal españoles para extranjeros NIE.
    /// </summary>
    public static string PatternTaxIdNieIndividual = @"(\b[XYZ]{1}[0-9]{7}[A-HJ-NP-Z]{1}\b)";

    /// <summary>
    /// Patrón de expresión regular que identifica códigos de identificación fiscal españoles para DNI.
    /// </summary>
    public static string PatternTaxIdDniIndividual = @"(\b[0-9]{1}[0-9]{7}[A-HJ-NP-Z]{1}\b)";

    /// <summary>
    /// Patrón de expresión regular que identifica códigos de identificación fiscal españoles presona física NIF.
    /// </summary>
    public static string PatternTaxIdNifIndividual = @"(\b[KLM]{1}[0-9]{7}[A-HJ-NP-Z]{1}\b)";

    /// <summary>
    /// Patrón de expresión regular que identifica códigos de identificación fiscal españoles persona jurídica NIF
    /// empresa.
    /// </summary>
    ...
    /// <summary>
    /// Patrón de expresión regular que identifica códigos de identificación fiscal españoles para no resiedentes y
    ...
    /// </summary>
    public string PersonTypeDen = string.Empty;
    ...
    /// </summary>
    public string DCStr = string.Empty;
    ...
    public string TaxCode
    {
      ...
      set
      {
        try
        {
          ...
          IsDCNum = int.TryParse(DCStr, out DCNum);
          if(IsDCNum)
          {
            DCStr = string.Empty;
          }
          strFirst = taxCode.Substring(0, 1);
          if(int.TryParse(strFirst, out numF))
          {
          ...
          switch(TaxIdentificationType)
          {
            case TaxIdEsTypes.DNI:
              ...
              {
                throw new TaxIdEsException($"TaxIdEs.DNI don't match the pattern: {rgxTaxIdDniIndividual}");
              }
              break;
            case TaxIdEsTypes.NIE:
              if(!rgxTaxIdNieIndividual.IsMatch(TaxCode))
              {
                throw new TaxIdEsException($"TaxIdEs.DNI don't match the pattern: {rgxTaxIdNieIndividual}");
              }
              break;
            case TaxIdEsTypes.NIF:
              switch(PersonType)
              {
                case TaxIdEsPersonTypeCode.Individual:
                  ...
                  {
                    throw new TaxIdEsException($"TaxIdEs.DNI don't match the pattern: {rgxTaxIdNifIndividual}");
                  }
                  break;
                case TaxIdEsPersonTypeCode.LegalEntity:
                  ...
                  {
                    throw new TaxIdEsException($"TaxIdEs.DNI don't match the pattern: {rgxTaxIdNifLegalEntity}");
                  }
                  break;
                default:
          ...
          if(TaxIdentificationType == TaxIdEsTypes.NIF)
          {
            IsDCOK = (IsDCNum) ?
                (GetDCNif() == (TaxIdEsDCNif)DCNum) :
                (GetDCNif() == (TaxIdEsDCNif)Enum.Parse(typeof(TaxIdEsDCNif), DCStr));
          }
          else
          {
            IsDCOK = (IsDCNum) ?
                (GetDCDniNie() == (TaxIdEsDCDniNie)DCNum) :
    ...
    /// <summary>
    /// Crea una instancia de la clase TaxIdEs utilizada para la validación de números de identificación fiscal y para
    /// la obtención de información del tipo de persona y ubicación de las personas jurídicas.
    /// </summary>
    ...
    /// </summary>
    /// <returns>
    /// Devualve un tipo TaxIdEsDCNif que repersenta un  número del 1 al 9, o bien una letra.
    /// </returns>
    ...
    /// </summary>
    /// <returns>
    /// Devualve un tipo TaxIdEsDCNif que repersenta  un número del 1 al 22, o bien una letra.
    /// </returns>
    private TaxIdEsDCDniNie GetDCDniNie()
    {
      try
      {
        int nCif = (TaxCode[0] > 57) ?  // Si es una letra entonces se trata de un NIE
            ((int)TaxCode[0] - (int)'X') * (int)Math.Pow(10, 7) + int.Parse(TaxCode.Substring(1, 7)) : // X=0, Y=1, Z=2
            int.Parse(TaxCode.Substring(0, 8));
