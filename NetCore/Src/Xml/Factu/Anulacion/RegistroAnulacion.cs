...
namespace VeriFactu.Xml.Factu.Anulacion
{
  ...
  public partial class RegistroAnulacion : Registro
  {
    ...
    /// de la Orden HAC/1177/2024 de 17 de octubre establece el contenido:</para> <para>b) Para el registro de
    /// facturación de anulación:</para> <para>1.º NIF del emisor.</para> <para>2.º Numero de factura y serie.</para>
    /// <para>3.º Fecha de expedición de la factura.</para> <para>4.º Huella del registro de facturación
    /// anterior.</para> <para>5.º Fecha, hora y huso horario de generación del registro.</para>
    /// </summary>
    /// <returns>
    ...
    /// <summary>
    /// <para>Indicador que especifica que se trata de la anulación de un registro de facturación de alta (o subsanación
    /// de un registro de facturación de anulación previo) cuando este no existe en la AEAT (o en el propio sistema
    /// informático, si así procede indicar). Si no se informa este campo se entenderá que tiene valor "N". Este campo
    /// forma parte del detalle de las circunstancias de generación de los registros de facturación.</para> <para>L4: S
    /// / N</para> <para>Alfanumérico (1)</para>
    /// </summary>
    ...
    /// de anulación subsanado tras haber sido rechazado en su remisión inmediatamente anterior, es decir, en el último
    /// envío que contenía ese registro de facturación de anulación rechazado. Si no se informa este campo se entenderá
    /// que tiene valor "N". Solo es necesario informarlo en caso de remisión voluntaria «VERI*FACTU».  Este campo forma
    /// parte del detalle de las circunstancias de generación de los registros de facturación.</para> <para>L4: S /
    /// N</para> <para>Alfanumérico (1)</para>
    /// </summary>
    ...
    /// <summary>
    /// <para>Indicador que especifica quién se ha encargado de generar materialmente el registro de facturación de
    /// anulación, cuyos datos identificativos deberán hacerse  constar dentro del grupo de campos "Generador", en su
    /// correspondiente sitio en función de si dispone de NIF o no. Si no se informa este campo, tampoco se deberá
    /// informar "Generador", considerándose que es el mismo que el emisor material de la factura que se anula. Por otro
    /// lado, se entiende que el obligado a generar el registro de facturación de anulación siempre es el mismo que el
    /// obligado a expedir la factura que en él se anula (incluyendo los casos de sucesión).</para> <para>Alfanumérico
    /// (1) L16:</para> <para>E: Expedidor (obligado a Expedir la factura anulada).</para> <para>D: Destinatario.</para>
    /// <para>T: Tercero</para>
    /// </summary>
    ...
    /// Artículo 137 de la Orden HAC/1177/2024 de 17 de octubre establece el contenido:</para> <para>b) Para el registro
    /// de facturación de anulación:</para> <para>1.º NIF del emisor.</para> <para>2.º Numero de factura y serie.</para>
    /// <para>3.º Fecha de expedición de la factura.</para> <para>4.º Huella del registro de facturación
    /// anterior.</para> <para>5.º Fecha, hora y huso horario de generación del registro.</para> <para>Alfanumérico
    /// (64)</para>
    /// </summary>
    [XmlIgnore]
