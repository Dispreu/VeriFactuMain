...
namespace VeriFactu.Xml.Factu.Alta
{
  ...
  public class RegistroAlta : Registro
  {
    ...
    /// </summary>
    private string _FechaHoraHusoGenRegistro;

    /// <summary>
    /// Encadenamiento con el registro anterior.
    /// </summary>
    private Encadenamiento _Encadenamiento;
    ...
    /// facturación de alta:</para> <para>1.º NIF del emisor.</para> <para>2.º Numero de factura y serie.</para>
    /// <para>3.º Fecha de expedición de la factura.</para> <para>4.º Tipo de factura.</para> <para>5.º Cuota
    /// total.</para> <para>6.º Importe total.</para> <para>7.º Huella del registro de facturación anterior.</para>
    /// <para>8.º Fecha, hora y huso horario de generación del registro.</para>
    /// </summary>
    /// <returns>
    /// Cadena de entrada para el cálculo del hash
    /// </returns>
    protected internal override string GetHashTextInput()
    {
    ...
    {
      string nif = HttpUtility.UrlEncode($"{IDFacturaAlta?.IDEmisorFactura}");
      string numserie = HttpUtility.UrlEncode($"{IDFacturaAlta?.NumSerieFactura}");
      string fecha = HttpUtility.UrlEncode($"{IDFacturaAlta?.FechaExpedicionFactura}");
      string importe = HttpUtility.UrlEncode($"{ImporteTotal}");
    ...
    /// <summary>
    /// Datos de identificación de factura expedida para operaciones de baja y consulta. Contiene del NIF del obligado a
    /// expedir la factura, el número de factura y la fecha.
    /// </summary>
    ...
    /// <summary>
    /// <para>Nombre-razón social del obligado a expedir la factura.</para> <para>Alfanumérico (120)</para>
    /// </summary>
    [XmlElement("NombreRazonEmisor", Namespace = Namespaces.NamespaceSF, Order = 4)]
    public string NombreRazonEmisor { get; set; }

    /// <summary>
    /// <para>Indicador que especifica que se trata de una subsanación de un registro de facturación de alta previamente
    /// generado, por lo que el contenido de este nuevo registro de facturación es el correcto y el que deberá tenerse
    /// en cuenta. Si no se informa este campo se entenderá que tiene valor "N" (Alta Normal-Inicial). Este campo forma
    /// parte del detalle de las circunstancias de generación de los registros de facturación.</para> <para>Alfanumérico
    /// (1) L4:</para> <para>S: Sí</para> <para>N: No</para>
    /// </summary>
    [XmlElement(Order = 5)]
    public string Subsanacion { get; set; }

    /// <summary>
    /// <para>Indicador que especifica que se está generando -para volverlo a remitir- un nuevo registro de facturación
    /// de alta subsanado tras haber sido rechazado en su remisión inmediatamente anterior, es decir, en el último envío
    /// que contenía ese registro de facturación de alta rechazado. Si no se informa este campo se entenderá que tiene
    /// valor "N". Solo es necesario informarlo en caso de remisión voluntaria «VERI*FACTU». Este campo forma parte del
    /// detalle de las circunstancias de generación de los registros de facturación.</para> <para>Alfanumérico (1)
    /// L17</para>
    /// </summary>
    [XmlElement(Order = 6)]
    ...
    /// <summary>
    /// <para>Fecha en la que se ha realizado la operación siempre que sea diferente a la fecha de expedición.</para>
    ...
    /// <summary>
    /// <para>Descripción del objeto de la factura.</para> <para>Alfanumérico (500)</para>
    /// </summary>
    [XmlElement("DescripcionOperacion", Namespace = Namespaces.NamespaceSF, Order = 13)]
    public string DescripcionOperacion { get; set; }

    /// <summary>
    /// <para>Factura simplificada Articulo 7.2 Y 7.3 RD 1619/2012.  Si no se informa este campo se entenderá que tiene
    /// valor  “N".</para> <para>Alfanumérico (1) L4:</para> <para>S: Sí</para> <para>N: No</para>
    /// </summary>
    [XmlElement(Order = 14)]
    ...
    /// <summary>
    /// <para>Factura simplificada Articulo 7.2 Y 7.3 RD 1619/2012.  Si no se informa este campo se entenderá que tiene
    /// valor  “N".</para> <para>Alfanumérico (1) L5:</para> <para>S: Sí</para> <para>N: No</para>
    /// </summary>
    [XmlElement(Order = 15)]
    public string FacturaSinIdentifDestinatarioArt61d { get; set; }

    /// <summary>
    /// <para>Identificador que especifica aquellas facturas con base o importe de la factura superior al umbral
    /// especificado. Este campo es necesario porque contribuye a completar el detalle de la tipología de la factura. Si
    /// no se informa este campo se entenderá que tiene valor “N”.</para> <para>Alfanumérico (1) L4:</para> <para>S:
    ...
    /// <summary>
    /// <para>Identificador que especifica si la factura ha sido emitida por un tercero o por el destinatario.</para>
    ...
    /// <summary>
    /// <para>Identificador que especifica si tiene minoración de la base imponible por la concesión de cupones,
    /// bonificaciones o descuentos cuando solo se expide el original de la factura. Este campo es necesario porque
    /// contribuye a completar el detalle de la tipología de la factura. Si no se informa este campo se entenderá que
    /// tiene valor  “N”.</para> <para>Alfanumérico (1) L4:</para> <para>S: Sí</para> <para>N: No</para>
    /// </summary>
    ...
    /// <summary>
    /// <para>Importe total de la cuota (sumatorio de la Cuota Repercutida y Cuota de Recargo de Equivalencia).</para>
    ...
    /// <summary>
    /// <para>Importe total de la factura.</para> <para>Decimal(12,2).</para>
    /// </summary>
    ...
    /// <summary>
    /// <para>Número de registro obtenido al enviar la autorización en materia de facturación o de libros registro a que
    /// se refiere la disposición adicional primera del Real Decreto que aprueba el Reglamento. Este campo forma parte
    /// del detalle de las circunstancias de generación del registro de facturación.</para> <para>La disposición
    /// adicional primera del Real Decreto 1007/2023, de 5 de diciembre, establece que, cuando exista una autorización
    /// concedida por el Departamento de Gestión Tributaria según el Reglamento de facturación (Real Decreto 1619/2012),
    /// y en virtud de la cual no se disponga de ciertos datos de facturación mencionados en el artículo 10.1 del nuevo
    /// Reglamento, no será obligatorio incluir dichos datos en los registros de facturación. En estos casos, deberá
    /// indicarse tal circunstancia en el registro.</para> <para>Alfanumérico(15)</para>
    /// </summary>
    ...
    /// <summary>
    /// <para>Identificación del acuerdo (resolución) a que se refiere el artículo 5 del Reglamento. Este campo forma
    /// parte del detalle de las circunstancias de generación del registro de facturación.</para> <para>El artículo 5
    /// del mismo Reglamento permite que el titular del Departamento de Inspección Financiera y Tributaria de la AEAT,
    /// previa solicitud del interesado, pueda resolver la no aplicación del reglamento en relación con ciertos sectores
    /// empresariales, profesionales o contribuyentes, cuando quede justificado por prácticas comerciales o
    /// administrativas del sector, o para evitar perturbaciones en el desarrollo de actividades económicas. También se
    /// contempla la no aplicación en operaciones con circunstancias técnicas excepcionales que imposibiliten el
    /// cumplimiento del reglamento</para> <para>Alfanumérico (16)</para>
    /// </summary>
    ...
    /// <summary>
    /// <para>Huella o «hash» de cierto contenido de este registro de facturación. Dicho contenido se detallará en la
    /// documentación correspondiente en la sede electrónica de la AEAT (documento de huella...).</para> <para>El
    /// Artículo 137 de la Orden HAC/1177/2024 de 17 de octubre establece el contenido:</para> <para>a) Para el registro
    /// de facturación de alta:</para> <para>1.º NIF del emisor.</para> <para>2.º Numero de factura y serie.</para>
    /// <para>3.º Fecha de expedición de la factura.</para> <para>4.º Tipo de factura.</para> <para>5.º Cuota
    /// total.</para> <para>6.º Importe total.</para> <para>7.º Huella del registro de facturación anterior.</para>
    /// <para>8.º Fecha, hora y huso horario de generación del registro.</para> <para>Alfanumérico (64)</para>
    /// </summary>
    [XmlIgnore]
    ...
    /// <summary>
    /// <para>Huella o «hash» de cierto contenido de este registro de facturación. Dicho contenido se detallará en la
    /// documentación correspondiente en la sede electrónica de la AEAT (documento de huella...).</para>
