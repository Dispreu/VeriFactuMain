...
namespace VeriFactu.Qrcode
{
  ...
  internal sealed class Encoder
  {
    ...
    /// </summary>
    /// <remarks>
    /// Encode "bytes" with the error correction level "ecLevel". The encoding mode will be chosen internally by
    /// chooseMode(). On success, store the result in "qrCode". <para/> We recommend you to use QRCode.EC_LEVEL_L (the
    /// lowest level) for "getECLevel" since our primary use is to show QR code on desktop screens. We don't need very
    /// strong error correction for this purpose. <para/> Note that there is no way to encode bytes in MODE_KANJI. We
    /// might want to add EncodeWithMode() with which clients can specify the encoding mode. For now, we don't need the
    /// functionality.
    /// </remarks>
    ...
    public static void Encode(string content, ErrorCorrectionLevel ecLevel, IDictionary<EncodeHintType, object> hints, QRCode qrCode)
    {
      ...
      int desiredMinVersion = (hints == null || hints.Get(EncodeHintType.MIN_VERSION_NR) == null) ?
          1 :
          (int)hints
    ...
    /// </summary>
    /// <remarks>
    /// Choose the best mode by examining the content. Note that 'encoding' is used as a hint; if it is Shift_JIS, and
    /// the input is only double-byte Kanji, then we return <see cref="Mode.KANJI"/>
    /// </remarks>
    /// <param name="content">content to examine</param>
    ...
    /// </summary>
    /// <remarks>
    /// Get number of data bytes and number of error correction bytes for block id "blockID". Store the result in
    /// "numDataBytesInBlock", and "numECBytesInBlock". See table 12 in 8.5.1 of JISX0510:2004 (p.30)
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Interleave "bits" with corresponding error correction bytes. On success, store the result in "result". The
    /// interleave rule is complicated. See 8.6 of JISX0510:2004 (p.37) for details.
    /// </remarks>
    internal static void InterleaveWithECBytes(
      BitVector bits,
      int numTotalBytes,
      int numDataBytes,
      int numRSBlocks,
