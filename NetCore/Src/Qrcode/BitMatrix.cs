...
namespace VeriFactu.Qrcode
{
  ...
  /// position, and y is the row position. The ordering is always x, y. The origin is at the top-left. <para/>
  /// Internally the bits are represented in a 1-D array of 32-bit ints. However, each row begins with a new int. This
  /// is done intentionally so that we can copy out a row into a BitArray very efficiently. <para/> The ordering of bits
  /// is row-major. Within each int, the least significant bits are used first, meaning they represent lower x values.
  /// This is compatible with BitArray's implementation.
  /// </remarks>
  ...
  internal sealed class BitMatrix
  {
    ...
    /// </summary>
    /// <remarks>
    /// This method is for compatibility with older code. It's only logical to call if the matrix is square, so I'm
    /// throwing if that's not the case.
    /// </remarks>
