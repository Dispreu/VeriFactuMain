...
namespace VeriFactu.Qrcode
{
  ...
  internal sealed class MatrixUtil
  {
    ...
    private static readonly int[][] POSITION_DETECTION_PATTERN = new int[][]
    {
      ...
      {
        1, 1, 1, 1, 1, 1, 1
      }, new int[] { 1, 0, 0, 0, 0, 0, 1 }, new int[] { 1, 0, 1, 1, 1, 0, 1 }, new int[] { 1, 0, 1, 1, 1, 0, 1 }, new int[] { 1, 0, 1, 1, 1, 0, 1 },
    ...
    private static readonly int[][] TYPE_INFO_COORDINATES = new int[][]
    {
      new int[] { 8, 0 }, new int[]
                          {
                            8, 1
                          }, new int[] { 8, 2 }, new int[] { 8, 3 }, new int[] { 8, 4 }, new int[] { 8, 5 }, new int[] { 8, 7 }, new int[] { 8, 8 }, new int[] { 7, 8 },
      new int[] { 5, 8 },
    ...
    /// </summary>
    /// <remarks>
    /// Build 2D matrix of QR Code from "dataBits" with "ecLevel", "version" and "getMaskPattern". On success, store the
    /// result in "matrix".
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Embed basic patterns. On success, modify the matrix. The basic patterns are: - Position detection patterns -
    /// Timing patterns - Dark dot at the left bottom corner - Position adjustment patterns, if need be
    /// </remarks>
    /// <param name="version">Version of the QR code, [1 .. 40]</param>
    /// <param name="matrix">Bytematrix in which the output will be stored</param>
    ...
    /// is modified See 8.10 of JISX0510:2004 (p.47) for how to embed version information.
    /// </remarks>
    ...
    /// <summary>
    /// Embed "dataBits" using "getMaskPattern".
    /// </summary>
    /// <remarks>
    /// Embed "dataBits" using "getMaskPattern". On success, the matrix is modified For debugging purposes, it skips
    /// masking process if "getMaskPattern" is -1. See 8.7 of JISX0510:2004 (p.38) for how to embed data bits.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Return the position of the most significant bit set (to one) in the "value". The most significant bit is
    /// position 32. If there is no bit set, return 0. Examples: - findMSBSet(0) =&gt; 0 - findMSBSet(1) =&gt; 1 -
    /// findMSBSet(255) =&gt; 8
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Calculate BCH (Bose-Chaudhuri-Hocquenghem) code for "value" using polynomial "poly". The BCH code is used for
    /// encoding type information and version information. Example: Calculation of version information of 7. f(x) is
    /// created from 7. - 7 = 000111 in 6 bits - f(x) = x^2 + x^2 + x^1 g(x) is given by the standard (p. 67) - g(x) =
    /// x^12 + x^11 + x^10 + x^9 + x^8 + x^5 + x^2 + 1 Multiply f(x) by x^(18 - 6) - f'(x) = f(x) * x^(18 - 6) - f'(x) =
    /// x^14 + x^13 + x^12 Calculate the remainder of f'(x) / g(x) x^2
    /// __________________________________________________ g(x) )x^14 + x^13 + x^12 x^14 + x^13 + x^12 + x^11 + x^10 +
    /// x^7 + x^4 + x^2 -------------------------------------------------- x^11 + x^10 + x^7 + x^4 + x^2 <para/> The
    /// remainder is x^11 + x^10 + x^7 + x^4 + x^2 Encode it in binary: 110010010100 The return value is 0xc94 (1100
    /// 1001 0100) <para/> Since all coefficients in the polynomials are 1 or 0, we can do the calculation by bit
    ...
    /// </summary>
    /// <remarks>
    /// Make bit vector of type information. On success, store the result in "bits". Encode error correction level and
    /// mask pattern. See 8.9 of JISX0510:2004 (p.45) for details.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Make bit vector of version information. On success, store the result in "bits". See 8.10 of JISX0510:2004 (p.45)
    /// for details.
    /// </remarks>
