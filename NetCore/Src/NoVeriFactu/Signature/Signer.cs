...
{
  /// <summary>
  /// Clase responsable de la firma de ficheros de  registros en arreglo a las especificaciones de VERI*FACTU.
  /// </summary>
  public class Signer
  {
    ...
    /// </summary>
    internal static readonly int PROV_RSA_AES = 24;
    ...
    /// <summary>
    /// Devuelve una nueva instancia de VerifactuSignedXml basada en el documento xml de entrada.
    /// </summary>
    /// <param name="xmlDocument">Documento a firmar.</param>
    /// <returns>
    /// Nueva instancia de VerifactuSignedXml basada en el documento xml de entrada.
    /// </returns>
    private VerifactuSignedXml GetVerifactuSignedXml(XmlDocument xmlDocument)
    {
      string signatureId = GetSignatureId();
      VerifactuSignedXml signedXml = new VerifactuSignedXml(xmlDocument, $"xmldsig-{signatureId}");
      signedXml.KeyInfo = KeyInfo;
      signedXml.SigningKey = SigningKey;
      Reference refInXml = GetInXmlReference(signatureId);
      signedXml.AddReference(refInXml);
      Reference refSignedProperties = GetSignedPropertiesReference(signatureId);
    ...
    private RSACryptoServiceProvider GetCertificateKey(X509Certificate2 certificate)
    {
      ...
      string exportedKeyMaterial = certificate.PrivateKey.ToXmlString(true); //Include Private Parameters
      #pragma warning restore SYSLIB0028
      #pragma warning disable CA1416
      RSACryptoServiceProvider key = new RSACryptoServiceProvider(new CspParameters(PROV_RSA_AES));
    ...
    /// <summary>
    /// Devuelve el objeto KeyInfo de la firma adecuado para el certificado de entrada.
    /// </summary>
    ...
    /// <summary>
    /// Devuelve la referencia correspondiente al documento xml sin firmar de entrada.
    /// </summary>
    ...
    /// <summary>
    /// Devuelve la referencia correspondiente a las  propiedades adicionales de la firma.
    /// </summary>
    /// <param name="signatureId">Identificador de la firma.</param>
    /// <returns>
    /// Referencia a incluir en la firma con la propiedades adicionales al documento de entrada a firmar.
    /// </returns>
    ...
    /// <summary>
    /// Devuelve los datos binarios de una cadena con el documentos xml de entrada firmado.
    /// </summary>
    ...
    private byte[] Sign(XmlDocument xmlDocument, string name = "RegistroAlta")
    {
      ...
      VerifactuSignedXml signedXml = GetVerifactuSignedXml(xmlDocument);
      RootTmp rootTmp = new RootTmp(xmlDocument, name);
      ...
      byte[] xmlSigned = null;
      using(MemoryStream msXmlSigned = new MemoryStream())
      {
    ...
    {
      string name = registro.GetType().Name;
      Dictionary<string, string> nms = new Dictionary<string, string>
      {
        { "sum1", Namespaces.NamespaceSF },
      };
      // Firmamos el registro de alta
      byte[] xml = new XmlParser().GetBytes(registro, nms);
