...
namespace VeriFactu.Blockchain
{
  ...
  public class Blockchain : SingletonByKey<Blockchain>
  {
    ...
    /// <summary>
    /// Devuelve la ruta de almacenamiento de la cadena de bloques.
    /// </summary>
    /// <param name="sellerID">
    /// Emisor al que pertenece la cadena de bloques a gestionar.
    /// </param>
    /// <returns>
    /// Ruta de almacenamiento de la cadena de bloques.
    /// </returns>
    private string GetBlockchainPath(string sellerID)
    {
      string dir = $"{Settings.Current.BlockchainPath}{sellerID}";
    ...
    /// <summary>
    /// Devuelve un encadenamiento con el último elemento de la cadena.
    /// </summary>
    /// <returns>
    /// Encadenamiento con el último elemento de la cadena.
    /// </returns>
    ...
    /// <summary>
    /// Almacena los datos del último elemento de la cadena en disco.
    /// </summary>
    private void WriteVar()
    {
      ...
      else
      {
        ...
        File.WriteAllText(
          BlockchainVarFileName,
          $"{CurrentID}{_CsvSeparator}" +    // 0
    ...
    /// <summary>
    /// Devuelve un texto para el archivo csv de control representando la inserción en la cadena de bloques con los
    /// datos necesarios.
    /// </summary>
    ...
    /// <summary>
    /// Añade los datos del último elemento al archivo de control de la cadena.
    /// </summary>
    /// <param name="csvLines">
    /// Líneas de control a incluir en el archivo csv de control.
    /// </param>
    ...
    /// <summary>
    /// Recupera los datos del archivo de la cadena de bloques previo a la inserción del último elemento.
    /// </summary>
    ...
    private void RestorePreviousData(
      string blockchainDataFileName,
      string blockchainDataPreviousFileName)
    {
      bool isFirstLink = CurrentID == 0; // Se trataba del primer eslabón de la cadena
      bool isFirstBlockPeriodLink = false; // Se trata del primer eslabón del periodo
      if(!File.Exists(blockchainDataPreviousFileName))
      {
      ...
      bool hasBlockchainDataPreviousFile = !(isFirstLink || isFirstBlockPeriodLink);
    ...
    /// <summary>
    /// Archivo que almacena el valor de las variables en curso de la cadena.
    /// </summary>
    public string BlockchainVarFileName => $"{BlockchainPath}_{SellerID}.csv";

    /// <summary>
    /// Archivo copia de seguridad que almacena una porción del Blockchain correspondiente a los movimientos de un mes
    /// excepto el último movimiento. Es la copia del archivo con el nombre BlockchainDataFileName antes del registro
    /// del último movimiento.
    /// </summary>
    public string BlockchainDataPreviousFileName => $"{BlockchainPath}{CurrentTimeStamp:yyyyMM}.PREV.csv";

    /// <summary>
    /// Archivo que almacena una porción del Blockchain correspondiente a los movimientos de un mes.
    /// </summary>
    ...
    /// <returns>
    /// Instancia correspondiente a la cadena de bloques de un emisor de facturas.
    /// </returns>
    ...
    public static void LoadBlockchainsFromDisk()
    {
      ...
      string[] dirs = Directory.GetDirectories(Settings.Current.BlockchainPath);
      foreach(string dir in dirs)
      {
        string sellerID = Path.GetFileName(dir);
        Blockchain blockchain = new Blockchain(sellerID);
        if(File.Exists(blockchain.BlockchainVarFileName))
        {
          string lineVarData = File.ReadAllText(blockchain.BlockchainVarFileName);
          string[] valuesVarData = lineVarData.Split(_CsvSeparator);
          string currentID = valuesVarData[0];
          string currentTimeStamp = valuesVarData[1];
          string huella = valuesVarData[2];
          string fechaExpedicionFactura = valuesVarData[3];
          string idEmisorFactura = valuesVarData[4];
          string numSerieFactura = valuesVarData[5];
    ...
    /// </summary>
    /// <param name="registro">
    /// Registro a eliminar. Sólo puede eliminarse el último elemento añadido.
    /// </param> 
    ...
    public void Delete(Registro registro)
    {
      ...
      {
        throw new InvalidOperationException(
          $"Se ha intentado borrar el registro" +
                          $" {registro.Huella} que no coincide con el último {Current.Huella}");
      }
      string blockchainDataFileName = BlockchainDataFileName;
      string blockchainDataPreviousFileName = BlockchainDataPreviousFileName;
      Remove();
