...
namespace Verifactu
{
  ...
  public interface IVfInvoice
  {
    ...
    /// <summary>
    /// Identificador del vendedor. Debe utilizarse el identificador fiscal si existe (NIF, VAT Number...). En caso de no
    /// existir, se puede utilizar el número DUNS  o cualquier otro identificador acordado.
    /// </summary>        
    ...
    /// <summary>
    /// Identidicador del comprador. Debe utilizarse el identificador fiscal si existe (NIF, VAT Number...). En caso de no
    /// existir, se puede utilizar el número DUNS  o cualquier otro identificador acordado.
    /// </summary>        
    ...
    /// <summary>
    /// Código del país del destinatario (a veces también denominado contraparte, es decir, el cliente) de la operación de
    /// la factura expedida. <para>Alfanumérico (2) (ISO 3166-1 alpha-2 codes)</para>
    /// </summary>        
    ...
    /// <summary>
    /// Establece la fecha operación en caso de que se necesaria.
    /// </summary>
    ...
    /// <summary>
    /// Válida los datos de un NIF mediante el servicio de validación de la AEAT.
    /// </summary>
    /// <param name="nif">NIF a validar.</param>
    /// <param name="name">Nombre asociado al NIF a validar.</param>
    /// <returns>
    /// Cadena con la descripción de los errores o null si todo es correcto.
    /// </returns>
  ...
  public class VfInvoice : IVfInvoice
  {
    ...
    /// </summary>
    private VeriFactu.Business.Invoice _Invoice;

    /// <summary>
    /// Lista de líneas de impuestos.
    /// </summary>
    private List<VeriFactu.Business.TaxItem> _TaxItems;

    /// <summary>
    /// Facturas rectificadas.
    /// </summary>
    private List<VeriFactu.Business.RectificationItem> _RectificationItems { get; set; }
    ...
    /// <summary>
    /// Devuelve una instancia de clase Invoice creada a partir de los datos de esta instancia.
    /// </summary>
    /// <returns>Instancia de clase Invoice.</returns>
    private VeriFactu.Business.Invoice GetInvoice()
    {
      VeriFactu.Business.Invoice result = new VeriFactu.Business.Invoice(InvoiceID, InvoiceDate, SellerID)
      {
      ...
      int buyerIDType = BuyerIDType == 0 ? 2 : BuyerIDType;
      int[] buyerIDTypes = new int[] { 2, 3, 4, 5, 6, 7 };
      if(Array.IndexOf(buyerIDTypes, buyerIDType) == -1)
      {
        throw new ArgumentException(
          $"El valor de InvoiceType '{BuyerIDType}' no es válido." +
    ...
    public void GetValidateQr(string path)
    {
      ...
      VeriFactu.Xml.Factu.Alta.RegistroAlta registro = _Invoice.GetRegistroAlta();
      // Obtenemos la imágen del QR
      byte[] bmQr = registro.GetValidateQr();
    ...
    {
      _Invoice = GetInvoice();
      VeriFactu.Business.InvoiceEntry entry = new VeriFactu.Business.InvoiceEntry(_Invoice);
    ...
    {
      string tax = string.IsNullOrEmpty(taxItem.Tax) ? "01" : taxItem.Tax;
      string taxScheme = string.IsNullOrEmpty(taxItem.TaxScheme) ? "01" : taxItem.TaxScheme;
      string taxType = string.IsNullOrEmpty(taxItem.TaxType) ? "S1" : taxItem.TaxType;
      string taxException = string.IsNullOrEmpty(taxItem.TaxException) ? "NA" : taxItem.TaxException;
      if(!Enum.TryParse(tax, out VeriFactu.Xml.Factu.Impuesto eTax))
      {
        throw new ArgumentException(
          $"El valor de Tax '{tax}' no es válido." +
      ...
      {
        throw new ArgumentException(
          $"El valor de TaxScheme '{taxScheme}' no es válido." +
      ...
      {
        throw new ArgumentException(
          $"El valor de TaxType '{taxType}' no es válido." +
      ...
      {
        throw new ArgumentException(
          $"El valor de TaxException '{taxException}' no es válido." +
    ...
    {
      _RectificationItems.Add(
        new VeriFactu.Business.RectificationItem
        {
    ...
    {
      VfInvoiceResult result = new VfInvoiceResult
      {
        ResultCode = "0"
      };
      _Invoice = GetInvoice();
    ...
    public IVfInvoiceResult Delete()
    {
      ...
      _Invoice = GetInvoice();
      VeriFactu.Business.InvoiceCancellation cancellation = new VeriFactu.Business.InvoiceCancellation(_Invoice);
      try
      {
        cancellation.Save();
      }
    ...
    {
      NifValidation nifVal = new NifValidation(nif, name);
      List<string> errors = nifVal.GetErrors();
